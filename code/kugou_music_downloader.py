...
class KuGouSearchWorker(SearchWorker):
    def __init__(self, **kwargs):
        ...
        self.hash_url = 'https://www.kugou.com/yy/index.php?r=play/getdata&hash={}'
...
class AsyncKuGouSearchWorker(KuGouSearchWorker):
    ...
    async def func(self, result: Dict[str, any], session: aiohttp.ClientSession) -> Item:
        item = None
        file_hash = result['FileHash']
        hash_url = self.hash_url.format(file_hash)
        try:
    ...
    async def main(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                tasks = [loop.create_task(self.func(result, session)) for result in results]
...
class KuGouMusicDownloader(MusicDownloader):
    def __init__(self, searcher: Type[SearchWorker] = AsyncKuGouSearchWorker, **kwargs):
